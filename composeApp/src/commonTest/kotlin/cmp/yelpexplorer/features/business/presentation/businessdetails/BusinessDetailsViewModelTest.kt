...
class BusinessDetailsViewModelTest {
    ...
    private class FakeBusinessDetailsUseCase(
        private val error: Exception? = null,
    ) : BusinessDetailsUseCase {
        override fun execute(businessId: String): Flow<Business> = flow {
            if (error != null) {
                throw error
            } else {
    ...
    fun `viewState is ShowLoading then ShowBusinessDetails`() = runTest {
        ...
        val viewModel = BusinessDetailsViewModel(
            ...
            businessDetailsUseCase = FakeBusinessDetailsUseCase(),
    ...
    fun `viewState is ShowLoading then ShowError`() = runTest {
        ...
        viewModel.viewState.test {
            ...
            assertEquals(
                expected = BusinessDetailsViewState.ShowError(error = expectedResult),
                actual = awaitItem(),
            )
            // awaitComplete() - NEVER COMPLETES
        }
